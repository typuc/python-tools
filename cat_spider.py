...
start = 20
url = 'http://cat.h.highso.com.cn/cat/r/t?domain=haixue-base-service&ip=All&date=20220413&reportType=day&op=history'
response = requests.get(url=url, headers=headers)
tree = etree.HTML(response.text)
div_list = tree.xpath('//*[@id="main-container"]/div[2]/div[2]/div/table[3]/tr')
#//*[@id="main-container"]/div[2]/div[2]/div/table[3]/tbody
for i in div_list:
    item = i.xpath('./td[1]/a[2]/text()')
    if item:
        item = i.xpath('./td[1]/a[2]/text()')[0]
        total = i.xpath('./td[2]/text()')[0]
        failure = i.xpath('./td[3]/text()')[0]
        min_ms = i.xpath('./td[6]/text()')[0]
        max_ms = i.xpath('./td[7]/text()')[0]
        avg_ms = i.xpath('./td[8]/text()')[0]
        line95_ms = i.xpath('./td[9]/text()')[0]
        line99_ms = i.xpath('./td[10]/text()')[0]
        std_ms = i.xpath('./td[11]/text()')[0]
        qps = i.xpath('./td[12]/text()')[0]
        print(item, total, failure, min_ms, max_ms, avg_ms, line95_ms, line99_ms, std_ms, qps)


# print(d1,d2,d3,d3,d4, d5)
# dfor i in div_list:
#     print(i)
# div_list = tree.xpath('//*[@id="main-container"]/div[2]/div[2]/div/table[3]/tr[2]/td[2]/text()')
